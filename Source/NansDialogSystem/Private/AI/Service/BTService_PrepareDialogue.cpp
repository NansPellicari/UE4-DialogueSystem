...
void UBTService_PrepareDialogue::OnBecomeRelevant(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	BTDialogPointsHandler = Cast<UBTDialogPointsHandler>(BlackboardComp->GetValueAsObject(Settings.PointsHandlerKey));
	...
	const auto PlayerHUD = NDialogBTHelpers::GetPlayerHUD(OwnerComp, FString(__FUNCTION__));
